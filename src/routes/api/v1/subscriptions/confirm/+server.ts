...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    if (payload?.action !== 'subscribe_confirm' || !payload?.email) {
    ...
    const existing = await prisma.subscription.findFirst({ where: { email_user: email } });
    if (!existing) {
      await prisma.subscription.create({ data: { email_user: email } });
    }

    return new Response(renderHtml('Berhasil mengonfirmasi berlangganan. Terima kasih!'), {
      status: 200,
...
function renderHtml(message: string) {
  ...
  <body>
    <div class="card">
      <h1 class="title">Konfirmasi Berlangganan</h1>
